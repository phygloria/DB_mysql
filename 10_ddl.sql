...
insert into tb1 values(2, 20, 'n', null);
describe tb1;
...
-- auto_increment가 걸려 있는 칼럼은 primary key 제거가 안되므로 auto_increment를 modify 명령어로 제거한다.
-- modify는 칼럼의 정의를 바꾸는 것이다.
alter table tb2
modify pk int;


-- 테이블의 primary key는 하나만 가질 수 있어 별도의 컬럼을 지정하지 않아도 삭제가 가능하다.
alter table tb2
drop primary key;
